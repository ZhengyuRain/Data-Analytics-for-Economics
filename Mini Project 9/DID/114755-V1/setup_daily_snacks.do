...
log using ../logs/setup_daily_snacks, text replace
...
bysort store txmonth txday: egen dollarsperday = total(nds_amt*(type==4))
bysort store txmonth txday: egen txsperday = total(diff_tx)
bysort store txmonth txday: egen itemsperday = total(type==4)
...
bysort store txmonth txday: egen cal1P=total(cal*(1-multiplepeople)*(type==4))
bysort store txmonth txday: egen items1P = total((1-multiplepeople)*(type==4))
bysort store txmonth txday: egen dollars1P = total(nds_amt*(1-multiplepeople)*(type==4))
...
save $bigdatadir/working/setup_daily_temp_snacks, replace
...
save $bigdatadir/working/setup_daily_snacks, replace
