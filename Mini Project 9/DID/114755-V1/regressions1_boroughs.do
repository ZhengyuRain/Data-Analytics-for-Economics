...
sort store
merge store using /data/gsb/SBC/stores
tab _merge
...
gen law_bronx = law*(county=="BRONX")
gen law_brooklyn = law*(county=="KINGS")
gen law_queens = law*(county=="QUEENS")
gen law_staten = law*(county=="RICHMOND")
gen law_manhattan = law*(county=="NEW YORK")
...
xi: areg meancal law_* $weather i.txdow i.txweek, absorb(store)
estimates store mean
xi: areg p50cal law_* $weather i.txdow i.txweek, absorb(store)
estimates store median
xi: areg p90cal law_* $weather i.txdow i.txweek, absorb(store)
estimates store pctile90
xi: areg p95cal law_* $weather i.txdow i.txweek, absorb(store)
estimates store pctile95

log using ../logs/regressions1_boroughs, text replace
