...
******************* Last Few Transaction level vars, only for drink and snack items: *********
bysort store txyear txmonth txday txid: egen txcal = total(cal*(type~=0)*(type~=5))
bysort store txyear txmonth txday txid: egen txcaldrinks = total(cal*(type==1))
bysort store txyear txmonth txday txid: egen txcalsnacks = total(cal*(type==2))

bysort store txyear txmonth txday txid: egen txitems = total(nds_qty*(type~=0)*(type~=5))
...
save $bigdatadir/working/ch/setup_sumstats1, replace
