...
log using ../logs/setup_daily_drinks, text replace
...
bysort store txmonth txday: egen dollarsperday = total(nds_amt*(type==1))
bysort store txmonth txday: egen txsperday = total(diff_tx)
bysort store txmonth txday: egen itemsperday = total(type==1)
...
bysort store txmonth txday: egen cal1P=total(cal*(1-multiplepeople)*(type==1))
bysort store txmonth txday: egen items1P = total((1-multiplepeople)*(type==1))
bysort store txmonth txday: egen dollars1P = total(nds_amt*(1-multiplepeople)*(type==1))
...
save $bigdatadir/working/setup_daily_temp_drinks, replace
...
save $bigdatadir/working/setup_daily_drinks, replace
