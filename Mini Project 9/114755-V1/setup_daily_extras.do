...
log using ../logs/setup_daily_extras, text replace
...
* ONLY EXTRAS
* don't keep soy modification
ge extra= (type==3 & cal>0)

* Revenue and transactions per Day:

bysort store txmonth txday: egen dollarsperday = total(nds_amt*extra)
bysort store txmonth txday: egen txsperday = total(diff_tx)
bysort store txmonth txday: egen itemsperday = total(extra)
...
bysort store txmonth txday: egen cal1P=total(cal*(1-multiplepeople)*extra)
bysort store txmonth txday: egen items1P = total((1-multiplepeople)*extra)
bysort store txmonth txday: egen dollars1P = total(nds_amt*(1-multiplepeople)*extra)
...
save $bigdatadir/working/setup_daily_temp_extras, replace
...
save $bigdatadir/working/setup_daily_extras, replace
