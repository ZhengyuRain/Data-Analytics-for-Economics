...
log using ../logs/setup_daily_drinksmore5cal, text replace
...
* KEEP ONLY DRINKS more than 5 cal
ge drinkmore5cal=(type==1 & cal>5)
* tea and coffee almost no cals
...
bysort store txmonth txday: egen dollarsperday = total(nds_amt*drinkmore5cal)
bysort store txmonth txday: egen txsperday = total(diff_tx)
bysort store txmonth txday: egen itemsperday = total(drinkmore5cal)
...
bysort store txmonth txday: egen cal1P=total(cal*(1-multiplepeople)*drinkmore5cal)
bysort store txmonth txday: egen items1P = total((1-multiplepeople)*drinkmore5cal)
bysort store txmonth txday: egen dollars1P = total(nds_amt*(1-multiplepeople)*drinkmore5cal)
...
save $bigdatadir/working/setup_daily_temp_drinksmore5cal, replace
...
save $bigdatadir/working/setup_daily_drinksmore5cal, replace
